...
namespace EASEncoder
{
    public static class EASEncoder
    {
        ...
        private static string _fileNameWithExtension = string.Empty;
        private static int _speechSpeed;
        ...
        public static void CreateNewMessage(EASMessage message, bool ebsTone = true, bool nwsTone = false,
            string announcement = "", string fileName = "", bool openFile = false, string voiceName = "", bool speechEnabled = false, int outputFileType = 0, int speechSpeed = 0, string outputPath = "", string fileNameWithExtension = "")
        {
            cancelled = false;
            ...
            if (openFile)
            {
                Debug.WriteLine(_fileName + (!String.IsNullOrWhiteSpace(Path.GetExtension(_fileNameWithExtension)) ? Path.GetExtension(_fileNameWithExtension) : ".wav"));

                
                // If MP3, select the MP3. If Wave, select the Wave.
                if (outputFileType == 2)
                    Process.Start("explorer.exe",
                        "/select, \"" + _outputPath + "\\" + _fileName + (!String.IsNullOrWhiteSpace(Path.GetExtension(_fileNameWithExtension)) ? Path.GetExtension(_fileNameWithExtension) : ".mp3") + "\"");
                else
                    Process.Start("explorer.exe",
                        "/select, \"" + _outputPath + "\\" + _fileName + (!String.IsNullOrWhiteSpace(Path.GetExtension(_fileNameWithExtension)) ? Path.GetExtension(_fileNameWithExtension) : ".wav") + "\"");

            }
        }

        public static MemoryStream GetMemoryStreamFromNewMessage(EASMessage message, bool ebsTone = true,
            bool nwsTone = false, string announcement = "", string voiceName = "", bool speechEnabled = false, int speechSpeed = 0)
        {
            _useEbsTone = ebsTone;
            _useNwsTone = nwsTone;
            if (speechEnabled)
        ...
        {
            /* If the user chooses to overwrite a file, Windows will warn them.
               If the user types a filename without the extension, Windows will not warn them.
               This is to warn the user of overwriting when Windows does not. */
            if (File.Exists(_outputPath + "\\" + _fileName + ".wav") && !Path.HasExtension(_fileNameWithExtension))
            {
                DialogResult dr = MessageBox.Show($"The file \"{_fileName}.wav\" already exists.\nDo you want to replace it?", "Confirm Save As", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            ...
            try
            {
                var f = new FileStream(_outputPath + "\\" + _fileName + 
                    (String.IsNullOrWhiteSpace(Path.GetExtension(_fileNameWithExtension)) ? ".wav" : Path.GetExtension(_fileNameWithExtension)),
                    FileMode.Create);
            ...
            {
                cancelled = true;
                MessageBox.Show("You do not have permission to write to this location.", "Error encoding EAS audio", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                cancelled = true;
                MessageBox.Show($"An unexpected error occurred: {ex.Message}", "Error encoding EAS audio", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private static void GenerateMp3File()
        {
            /* If the user chooses to overwrite a file, Windows will warn them.
               If the user types a filename without the extension, Windows will not warn them.
               This is to warn the user of overwriting when Windows does not. */
            if (File.Exists(_outputPath + "\\" + _fileName + ".mp3") && !Path.HasExtension(_fileNameWithExtension))
            {
                DialogResult dr = MessageBox.Show($"The file \"{_fileName}.mp3\" already exists.\nDo you want to replace it?", "Confirm Save As", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dr == DialogResult.No)
                {
                    cancelled = true;
                    return;
            ...
            try
            {
                ...
                using (var writer = new LameMP3FileWriter(_outputPath + "\\" + _fileName +
                    (String.IsNullOrWhiteSpace(Path.GetExtension(_fileNameWithExtension)) ? ".mp3" : Path.GetExtension(_fileNameWithExtension)),
                    reader.WaveFormat, LAMEPreset.VBR_90)) // ABR_32 is the old version
        ...
        private static byte[] GenerateVoiceAnnouncement(string announcement)
        {
            ...
            var firstOrDefault = synthesizer.GetInstalledVoices()
                .FirstOrDefault(x => x.VoiceInfo.Name.ToLower().Contains(_voiceName.ToLower()));
